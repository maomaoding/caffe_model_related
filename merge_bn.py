...
def merge_conv_and_bn(net, i_conv, i_bn, i_scale):
  ...
  kye_scale = net._layer_names[i_scale] if i_scale else None
  ...
  if net.params.has_key(kye_scale):
    print 'Combine {:s} + {:s} + {:s}'.format(key_conv, key_bn, kye_scale)
    scale_weight = copy_double(net.params[kye_scale][0].data)
    scale_bias = copy_double(net.params[kye_scale][1].data)
    net.params[kye_scale][0].data[:] = 1
    net.params[kye_scale][1].data[:] = 0
  else:
    ...
    for i, layer in enumerate(net.layers):
        ...
        for j in xrange(i - 1, -1, -1):
            ...
            if l_bottom in tops_of_j:
                if net.layers[j].type not in ['Convolution']:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_weights', default=None)
    ...
    dst_net = caffe.Net(dst_deploy_name, org_weight_name, caffe.TEST)

    dst_net.save(dst_weight_name)